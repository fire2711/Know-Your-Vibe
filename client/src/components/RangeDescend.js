...
const RangeDescend = () => {
  ...
  const handleConfirm = async () => {
    try {
      const response = await fetch('http://127.0.0.1:5000/api/pairint-descending', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ start: value[0], end: value[1] }),
      });
      const data = await response.json();
      setDRange(data.range_songs_descending);
    } catch (error) {
